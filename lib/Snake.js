...
class Snake extends GamePiece {
  constructor(x, y, height = 20, width = 20, color, dx, dy, dxv){
    ...
    this.head = null;
    this.tail = [
      // new GamePiece(10, 20, 20, 20),
      // new GamePiece(30, 20, 20, 20),
      // new GamePiece(40, 20, 20, 20),
      // new GamePiece(50, 20, 20, 20)
    ];
    this.totalTail = 4;
    this.lives = 3;
    this.score = 0;
  ...
  // offSethead(xOffSet, yOffSet, newTail, head) {
  //   newTail.x = head.x += xOffSet;
  //   newTail.y = head.y += yOffSet;
  //   return this;
  // }
  makeInitialSnake() {
  ...
  drawSnake(context) {
    ...
    this.tail.forEach((block) => {
      block.draw(context)
      // context.fillRect(this.tail.x, this.tail.y, 20, 20)
    })
  }
  ...
  moveSnake() {
    ...
    if (this.tail.length > this.totalTail) {
      this.tail.pop();
    }
  }

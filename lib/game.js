...
class Game {
  constructor() {
    ...
    this.score = 0;
    this.highScore = 0;
    this.snake = new Snake(150, 20, 20, 20, 'rgb(255, 0, 0)', 0);
    this.food = new Food(150, 20, 20, 20, 'rgb(0, 255, 0)');
    this.blocks = [
      this.snake,
      this.food
    ];
  ...
  detectCollisionBorder(context) {
    if (this.snake.head.x  < 20 || this.snake.head.x > 360 || this.snake.head.y < 20 || this.snake.head.y > 560) {
      this.snake.death();
